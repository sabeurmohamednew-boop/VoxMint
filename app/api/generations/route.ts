...
export async function POST(request: Request) {
  ...
  try {
    ...
    const failureMode = isE2eTestAuthEnabled()
      ...
      : null;
    const testGenerationRateLimit = isE2eTestAuthEnabled() && request.headers.get("x-e2e-rate-limit-probe") === "generation"
      ? getEnv().E2E_GENERATIONS_PER_MINUTE
      : undefined;
    return ok({ generation: await generateForUser(user.id, await request.json(), requestIp(request), id, failureMode, testGenerationRateLimit) }, id, { status: 201 });
  } catch (error) {
    return apiError(error, id);
  }
}
