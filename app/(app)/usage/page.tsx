...
export default async function UsagePage() {
  ...
  return (
    <>
      ...
      <div className="mt-5 flex gap-3 rounded-[12px] border border-[var(--info)]/25 bg-[var(--info)]/5 p-4 text-sm leading-6 text-[var(--foreground-secondary)]"><Info className="mt-0.5 h-5 w-5 shrink-0 text-[var(--info)]" /><p>Only successful, committed operations recorded by this VoxMint deployment count here. Failed requests release reservations. These totals are not a live Cartesia subscription balance.</p></div>
    </>
  );
}
