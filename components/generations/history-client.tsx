...
export function HistoryClient({ initialGenerations, voices, initialProvider }: { initialGenerations: GenerationDto[]; voices: VoiceDto[]; initialProvider: HistoryProviderFilter }) {
  ...
  const actions = (item: GenerationDto) => <ActionButtons item={item} downloading={downloadingId === item.id} onPlay={() => setSelected(item)} onDetails={() => setDetails(item)} onDownload={() => void downloadItem(item)} onRename={() => setRename(item)} onDelete={(trigger) => { deleteTriggerRef.current = trigger; setRemove(item); }} />;
  ...
  return <>
    ...
    <ConfirmDialog open={Boolean(remove)} onOpenChange={(open) => { if (!open) setRemove(null); }} title="Delete stored audio and history record?" description="The generated audio object and this VoxMint history record will no longer be available." confirmLabel="Delete audio and record" danger onConfirm={deleteItem} returnFocus={() => deleteTriggerRef.current} />
  </>;
}
