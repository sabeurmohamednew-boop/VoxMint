  ...
  return (
    <section id="clone" className="clone-panel panel p-5 sm:p-[22px]" aria-labelledby="clone-title">
      ...
      <form className="mt-5 space-y-4" onSubmit={submit}>
        ...
        <input ref={inputRef} className="sr-only" type="file" aria-label="Choose audio sample" accept=".flac,.mp3,.mpeg,.mpga,.oga,.ogg,.wav,.webm,audio/flac,audio/mpeg,audio/ogg,audio/wav,audio/webm" onChange={(event) => selectFile(event.target.files?.[0])} />
        {!state.file ? (
          <div
        ...
        <div className="grid gap-3 sm:grid-cols-2"><div><label htmlFor="voice-language" className="text-[13px] font-semibold">Primary language</label><select id="voice-language" className="field mt-2 px-3" value={language} onChange={(event) => setLanguage(event.target.value)}>{SUPPORTED_LANGUAGES.map((option) => <option key={option.code} value={option.code}>{option.label}</option>)}</select></div><div className="flex items-end">{!showDescription ? <button type="button" className="button-ghost w-full" onClick={() => setShowDescription(true)}><Plus className="h-4 w-4" />Add description</button> : <label className="w-full text-[13px] font-semibold">Description<input className="field mt-2 px-3" value={description} onChange={(event) => setDescription(event.target.value)} maxLength={160} /></label>}</div></div>
