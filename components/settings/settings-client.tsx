...
export function SettingsClient({ account, operations }: { account: Account; operations: Operations }) {
  ...
  async function save(event: React.FormEvent) {
    ...
    setBusy(true);
    try {
      await fetchJson("/api/account", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...form, retentionDays: operations.retentionWorkerEnabled && form.retentionDays !== "none" ? Number(form.retentionDays) : null }) });
  ...
  return <>
    <form onSubmit={save} className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
      <section className="panel p-5 sm:p-6"><h2 className="text-base font-semibold">Profile &amp; preferences</h2><div className="mt-5 grid gap-5 sm:grid-cols-2">
        ...
        <label className="text-[13px] font-semibold">Default language<select className="field mt-2 px-3" value={form.preferredLanguage} onChange={(event) => setForm({ ...form, preferredLanguage: event.target.value })}><option value="en">English</option><option value="fr">French</option><option value="ar">Arabic</option></select></label>
        ...
        <label className="text-[13px] font-semibold">Retention preference<select className="field mt-2 px-3" value={operations.retentionWorkerEnabled ? form.retentionDays : "none"} disabled={!operations.retentionWorkerEnabled} onChange={(event) => setForm({ ...form, retentionDays: event.target.value })}><option value="none">Keep until I delete</option><option value="7">7 days</option><option value="30">30 days</option><option value="90">90 days</option></select></label>
      </div><p className="mt-3 text-[11.5px] leading-5 text-[var(--muted)]">{operations.retentionWorkerEnabled ? "Automated retention is active for this deployment." : "Automated retention is not active. Delete audio manually from History."}</p><button className="button-primary mt-5 px-5" disabled={busy}><Save className="h-4 w-4" />{busy ? "Saving…" : "Save settings"}</button></section>
